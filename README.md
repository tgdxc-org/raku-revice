# プロジェクト概要
らくらく → EVENT NOVAへのリバイスにあたって作成したランディングページ
特徴としては、アプリ本体と同じmicroCMSコンテンツを参照して、自動スライドを実装していること
アプリ本体と同じように、ビルド時にAPIでmicroCMSのデータを参照してビルドする
同じデータ、同じIDなので、シンプルな実装でリンク先を設定できる

# ADR-001: <CMSをデータ元とした自動スライダー>

## Status
- 採用

## Context（背景）
- なぜこの判断が必要になったか
ファーストビューで会場の豊富さを見せたいという要望が起点

- 前提条件・制約
アプリへのコンバージョンを高めたい
ファーストビューには地域ごとの遷移リンクを配置する（目立たせる）

- 現在の構成・状況
microCMSのデータを参照し、リンクを接続

## Decision（決定）
- 何を選んだか（簡潔に）
microCMSからデータ取得、自動スライダーを表示

## Reason（理由）
- この判断を選んだ根拠
ファーストビューに収まる範囲で複数の会場をユーザーに見せる手段として、自動スライド
興味を惹くものがあれば、クリックで直接コンバージョンできる導線設計

- 重視した判断軸
実装範囲のコンパクトさと会場を豊富に見せることのバランス

## Consequences（結果・影響）
- 想定されるリスク
アプリ本体＋ランディングページで同じデータを参照することから、ビルド回数の増加が想定される
当時はコスト感や更新頻度が不透明だったことから、Webhook連携を行っていない

# ADR-002: <施工主体からアプリ主体の説明に変更>

## Status
- 採用

## Context（背景）
- なぜこの判断が必要になったか
会場検索アプリそのものの説明が乏しかった

- 前提条件・制約
アプリへのコンバージョンを高めたい

## Decision（決定）
- 何を選んだか（簡潔に）
アプリ主体の説明ページに刷新

## Reason（理由）
- この判断を選んだ根拠
アプリへの導線は設置されているものの、クリックした際に何が起こるのかが読めないつくりになっていた

- 重視した判断軸
不安を取り除くこと
クリックした後の挙動がユーザーに予測できること
（怪しいリンクは踏まないよねとフィードバック頂いた）

# ADR-003: <各会場データ、サムネイル画像の取り扱いについて>

## Status
- 採用

## Context（背景）
- なぜこの判断が必要になったか
エックスサーバーの用途がなくなったことから、ファイルの管理場所の変更を行った

- 前提条件・制約
サムネイル画像のURLはCMS側で管理しており、アプリ用途の為に相対パスを保管している
アプリ本体側では、リポジトリ内に格納している

## Decision（決定）
- 何を選んだか（簡潔に）
LP側ではアプリのドメイン＋相対パスの形でURLを設定

## Reason（理由）
- この判断を選んだ根拠
サムネイル画像の管理場所を1か所に統一することで、更新フローの重複を避ける

- 重視した判断軸
同じ作業をしない